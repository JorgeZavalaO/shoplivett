...
| Incidencia `RETURN + CREDIT` y cancelacion | Creditos creados por incidencia se revierten o bloquean si corresponde. | Incidencias, creditos | `AUD-DATA-002` | Integracion | No queda credito disponible por incidencia cancelada. |
| Dano/perdida con stock reservado | No se consume stock reservado o vendido. | Incidencias, inventario | `AUD-DATA-006` | Integracion | La accion falla si no hay disponible real. |
...
| Ajuste manual concurrente | Dos ajustes simultaneos no pierden updates ni rompen invariantes. | Inventario | `AUD-DATA-005` | Integracion/concurrencia | Resultado final equivale a ambos ajustes o uno falla con conflicto controlado. |
