...
- **Recepción automática**: al cambiar a `COMPLETE`, todos los productos pendientes se reciben automáticamente (`quantityReceived` = `quantityPurchased`), se incrementa `quantityAvailable` y se sincroniza el stock. La `estimatedArrivalDate` se actualiza a la fecha actual.
