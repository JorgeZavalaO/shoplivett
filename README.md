...
- **Cambio rápido de estado**: dropdown en la página de detalle con los estados disponibles. Al seleccionar uno, se confirma y ejecuta directamente sin abrir el formulario de edición completo.
- **Recepción automática**: al cambiar a `COMPLETE`, todos los productos pendientes se reciben automáticamente (`quantityReceived` = `quantityPurchased`), se incrementa `quantityAvailable`, se registra un movimiento `IN` en el historial de inventario y se sincroniza el stock. La `estimatedArrivalDate` se actualiza a la fecha actual.
